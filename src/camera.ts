...
class Camera extends Element {
    ...
    getMyRay(screenX: number, screenY: number, ray: Ray) {
        const { ortho, scene } = this;
        const cameraPos = this.mainCamera.getPosition();

        // create the pick ray in world space
        if (ortho) {
            this.camera.screenToWorld(screenX, screenY, -1.0, vec);
            this.camera.screenToWorld(screenX, screenY, 1.0, vecb);
            vecb.sub(vec).normalize();
            ray.set(vec, vecb);
        } else {
            this.camera.screenToWorld(screenX, screenY, 1.0, vec);
    ...
    // TODO 根据intersect改造
    // 获取当前帧像素对应的世界坐标
    async getWorldPointsInCurrentFrame() {
        const { scene } = this;
